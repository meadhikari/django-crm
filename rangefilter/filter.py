...
class DateRangeFilter(admin.filters.FieldListFilter):
    def __init__(self, field, request, params, model, model_admin, field_path):
        ...
        #params = {'date__lte':'2073-11-8', 'date__gte': '2046-7-23'}
        #params = {'date__lte': '1989-11-9', 'date__gte': '2046-7-23'}
        #print self.lookup_kwarg_gte
        '''
        try:
            converter = NepaliDateConverter()
            date__gte = '-'.join(list(map(str,list(converter.bs2ad(tuple(map(int,params['date__gte'].split('-'))))))))
            date__lte = '-'.join(list(map(str,list(converter.bs2ad(tuple(map(int,params['date__lte'].split('-'))))))))
    ...
    def _make_query_filter(self, validated_data):
        ...
        converter = NepaliDateConverter()
        try:
            date_value_gte = datetime.datetime.strptime('-'.join(list(map(str, list(converter.bs2ad(tuple(map(int, str(validated_data.get(self.lookup_kwarg_gte, None)).split('-')))))))), '%Y-%m-%d')
            date_value_lte = datetime.datetime.strptime('-'.join(list(map(str, list(converter.bs2ad(tuple(map(int, str(validated_data.get(self.lookup_kwarg_lte, None)).split('-')))))))), '%Y-%m-%d')
